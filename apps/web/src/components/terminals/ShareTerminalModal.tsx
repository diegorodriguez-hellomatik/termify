...
import { shareApi, TerminalShare, SharePermission } from '@/lib/api';

interface ShareTerminalModalProps {
  isOpen: boolean;
  onClose: () => void;
  terminalId: string;
  terminalName: string;
...
export function ShareTerminalModal({
  isOpen,
  onClose,
  terminalId,
  terminalName,
  ...
  // Link share state
  const [linkShare, setLinkShare] = useState<TerminalShare | null>(null);
  ...
  const loadShares = useCallback(async () => {
    ...
    try {
      const response = await shareApi.getShares(terminalId, token);
      if (response.success && response.data) {
        ...
        if (existing) {
          ...
          setShareUrl(
            `${window.location.origin}/share/${existing.shareToken}`
          );
        }
      } else {
  ...
  const handleShareByEmail = async (e: React.FormEvent) => {
    ...
    try {
      const response = await shareApi.shareWithEmail(
        terminalId,
  ...
  const handleUpdateSharePermission = async (
    ...
    try {
      const response = await shareApi.updateShare(
        terminalId,
        shareId,
        { permission },
        token
  ...
  const handleRevokeShare = async (shareId: string) => {
    ...
    try {
      const response = await shareApi.deleteShare(terminalId, shareId, token);
      if (response.success) {
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p
                    ...
                    style={{ color: isDark ? '#888' : '#666' }}
                  >
                    Anyone with the link can access this terminal
                  </p>
                </div>
              </div>
